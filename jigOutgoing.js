document.addEventListener("DOMContentLoaded", () => {
    ...
    async function lookupItemDescriptionFromAPI(serialValue) {
        ...
        try {
            ...
            const response = await fetch(
                `../php/jigOutgoing.php?serial=${encodeURIComponent(serialValue)}`,
            ...
            if (data.status === "success" && data.found) {
                ...
                // For outgoing, also get the current usage count from incoming
                if (data.usageCount) {
                    usageCountInput.value = data.usageCount;
                }
    ...
    form.onsubmit = async (e) => {
        ...
        try {
            const response = await fetch("../php/jigOutgoing.php", {
                method: "POST",
                body: formData
            });
    ...
    console.log('✅ JIG Outgoing form initialized - Optimized mode');
});